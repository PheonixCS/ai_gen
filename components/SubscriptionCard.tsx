...
const SubscriptionCard = () => {
  ...
  const features = [
    {
      ...
      description: "редактируй фото как профи"
    },
    {
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    },
    {
      ...
      description: "Преобразуйте свои фотографии"
    },
    {
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      description: "Получайте лучшее качество"
    }
  ];

  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % (features.length - 1));
    }, 3000);
  ...
  return (
    <CardContainer>
      <Title>Оплатите доступ к сервису</Title>
      ...
      <PriceContainer>
        <PriceInfo>
          <div>3 дня тест</div>
          <div>
            <Strikethrough>390 ₽</Strikethrough> <HighlightedPrice>1 ₽</HighlightedPrice>
          </div>
        </PriceInfo>
        <DiscountBadge>Выгода 90%</DiscountBadge>
      </PriceContainer>
      
      <PayButton disabled={!checked1 || !checked2}>Оплатить 1 ₽</PayButton>
      ...
      <CheckboxContainer>
        <CheckboxLabel>
          <CheckboxInput 
            type="checkbox" 
            checked={checked1} 
            onChange={(e: { target: { checked: boolean | ((prevState: boolean) => boolean); }; }) => setChecked1(e.target.checked)} 
          />
          Продолжая использование приложения, вы соглашаетесь с условиями использования и Политикой конфиденциальности
        </CheckboxLabel>
        <CheckboxLabel>
          <CheckboxInput 
            type="checkbox" 
            checked={checked2} 
            onChange={(e: { target: { checked: boolean | ((prevState: boolean) => boolean); }; }) => setChecked2(e.target.checked)} 
          />
...
const SliderGradientLeft = styled.div`
  position: absolute;
  left: 0;
  top: 0;
  bottom: 0;
...
const SliderGradientRight = styled.div`
  position: absolute;
  right: 0;
  top: 0;
  bottom: 0;
...
const PriceContainer = styled.div`
  ...
  border-radius: 12px;
  border: 1px solid;
  border-image: linear-gradient(90deg, #FFFBA1 0%, #58E877 100%) 1;
  margin-bottom: 24px;
`;
...
const PayButton = styled.button`
  width: 100%;
  padding: 16px;
  background: linear-gradient(90deg, #FFFBA1 0%, #58E877 100%);
  border: none;
  border-radius: 12px;
  font-weight: 600;
  font-size: 16px;
